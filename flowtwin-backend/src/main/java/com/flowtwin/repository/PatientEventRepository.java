...
public interface PatientEventRepository extends JpaRepository<PatientEvent, UUID> {
    ...
    long countByTypeAndOccurredAtGreaterThanEqualAndOccurredAtLessThan(EventType type, Instant from, Instant to);
    Optional<PatientEvent> findFirstByTypeOrderByOccurredAtAsc(EventType type);
}
